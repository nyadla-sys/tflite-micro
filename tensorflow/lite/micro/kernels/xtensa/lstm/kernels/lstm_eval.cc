...
#include "tensorflow/lite/micro/kernels/xtensa/lstm/helper_utils/tensor_utils.h"
...
void CalculateLstmGateHybrid(
  ...
  if (!is_aux_input_all_zeros) {
    tensor_utils::MatrixBatchVectorMultiplyAccumulate(
        ...
        aux_input_to_gate_row_sums, compute_row_sums, scratch0);
  }
  // For each batch and cell: compute recurrent_weight * output_state.
  ...
  if (!is_output_state_all_zeros) {
    if (recurrent_to_gate_weights_ledger != nullptr) {
      ...
      tensor_utils::MatrixBatchVectorMultiplyAccumulate(
          ...
          recurrent_to_gate_row_sums, compute_row_sums, scratch0);
    }
  }
  // For each batch and cell: compute cell_weight .* cell_state (peephole LSTM)
  if (use_peephole) {
    ...
    if (!tensor_utils::IsZeroVector(scratch0, n_batch * n_cell)) {
      ...
      if (projection_weights_ledger != nullptr) {
        ...
        tensor_utils::MatrixBatchVectorMultiplyAccumulate(
            ...
            projection_weights_row_sums, compute_row_sums, scratch2);
      }
    }
    if (proj_clip > 0.0f) {
      tensor_utils::CwiseClipping(output_state, n_batch * n_output, proj_clip);
...
// For each batch and cell: compute recurrent_weight * output_state.
  ...
  switch (activation) {
    case kTfLiteActSigmoid:
      ...
      tensor_utils::ApplySigmoid(inp, n_batch, 65536, tflm);
...
// Note: unlike float/hybrid, the activation is always Tanh.
  ...
  for (int j = 0; j < 7; j++) {
    for (int it = -32768; it <= 32767; it++) inp[it + 32768] = it;
    tensor_utils::ApplyTanh(j, inp, n_batch, 65536, tflm);
