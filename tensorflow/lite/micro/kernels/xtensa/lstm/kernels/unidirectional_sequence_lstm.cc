...
#include "tensorflow/lite/micro/kernels/xtensa/lstm/helper_utils/portable_tensor_utils.h"
...
TfLiteStatus CheckInputTensorDimensions(TfLiteContext* context,
  ...
  TF_LITE_ENSURE_OK(
      context,
      GetInputSafe(context, node, micro::lstm::full::kForgetGateBiasTensor,
                   &forget_gate_bias));
  ...
  TF_LITE_ENSURE_OK(
      context,
      GetInputSafe(context, node, micro::lstm::full::kOutputGateBiasTensor,
                   &output_gate_bias));
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_OK(
      context,
      GetOutputSafe(context, node, micro::lstm::full::kOutputTensor, &output));
  ...
  node->temporaries->data[0] = 0;
  TFLITE_DCHECK(context->RequestScratchBufferInArena != nullptr);
  context->RequestScratchBufferInArena(context, node->temporaries->size, (int *)&scratch_tensor_index);
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_OK(
      context,
      GetInputSafe(context, node, micro::lstm::full::kCellGateBiasTensor,
                   &cell_gate_bias));
  const TfLiteTensor* output_gate_bias;
  TF_LITE_ENSURE_OK(
      context,
  ...
  const TfLiteTensor* input_layer_norm_coefficients =
      use_layer_norm ? GetOptionalInputTensor(
                           context, node,
  ...
  const TfLiteTensor* cell_layer_norm_coefficients =
      use_layer_norm
          ? GetInput(context, node,
                     micro::lstm::full::kCellLayerNormCoefficientsTensor)
          : nullptr;
  const TfLiteTensor* output_layer_norm_coefficients =
      use_layer_norm
          ? GetInput(context, node,
      ...
      if (is_hybrid) {
        ...
        */
        ...
        for (int scratch_index = 0; scratch_index < 6; ++scratch_index) {
          ...
          context->GetScratchBuffer(context, node->temporaries->data[scratch_index]));
          scratch[scratch_index] = (TfLiteTensor *)scratch_tensor;
		}
        /*
                                TF_LITE_ENSURE_OK(context,
