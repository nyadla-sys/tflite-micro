...
#include "tensorflow/lite/micro/kernels/xtensa/lstm/helper_utils/portable_tensor_utils_impl.h"
...
void PortableApplyLayerNormFloat(const int16_t* input,
  ...
  const float bias_scale = (float)std::pow(two, -10) * layer_norm_scale;
  ...
  for (int batch = 0; batch < n_batch; ++batch) {
    ...
    if (variance == 0) {
      stddev_inv = 1.0f / (float)sqrt(1e-8);
    } else {
    ...
    for (int i = 0; i < n_input; ++i) {
      ...
      const int32_t quant_output = static_cast<int32_t>(
          std::round(weighted_normalized_value * (float)std::pow(2, 12)));
  ...
  for (int batch = 0; batch < n_batch; ++batch) {
    for (int i = 0; i < n_input; ++i) {
      ...
      const int32_t quant_output =
          static_cast<int32_t>(float_output * (float)std::pow(2, 15));
